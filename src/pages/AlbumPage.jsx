...
import { getAlbumByType, getTrackByAlbum } from "../api/api1";
...
function AlbumPage() {
  ...
  useEffect(() => {
    const fetchAlbumList = async () => {
      const res = await getAlbumByType("all");
  ...
  const onGoTrack = async (id) => {
    let res = await getTrackByAlbum(id, "all");
    setDataList(res.data);
    setIsAlbum(false);
  };

  const onBack = async () => {
    let res = await getAlbumByType("all");
    setDataList(res.data);
    setIsAlbum(true);
  };
    ...
    <div>
      <Container>
        <div className="page">
          ...
          {isAlbum ? (
            <div className="title">Explore Album</div>
          ) : (
                    ...
                    <>
                      {dataList.map((item, index) => (
                        <Album
                          key={index}
                          item={item}
                          onGoMusicTrack={(id) => onGoTrack(id)}
                          type="dummy"
                        />
                      ))}
                    </>
